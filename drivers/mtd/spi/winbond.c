...
int winbond_protect(struct spi_flash *flash, int on)
{
    struct spi_slave *spi = flash->spi;
    int ret;
    u8 status1, status2;
...
void spi_flash_reset_4byte_winbond(struct spi_slave *spi, u8 four_byte)
{
	u8 idcode[5];
	if (four_byte) {
		spi_flash_cmd(spi, 0x15, idcode, 1);
		if (idcode[0] & 0x01)
...
int inline spi_flash_checksize(const struct winbond_spi_flash_params *params)
{
	return params->nr_blocks * params->sectors_per_block * SIZE_4K;
}

static struct winbond_spi_flash *reset_spi;

void spi_flash_soft_reset(void)
{
	struct winbond_spi_flash *stm =reset_spi;
	if (spi_flash_checksize(stm->params) > SIZE_16M)
		spi_flash_reset_4byte_winbond(stm->flash.spi, 0);
}
...
struct spi_flash *spi_flash_probe_winbond(struct spi_slave *spi, u8 *idcode)
{
	...
	if (spi_flash_checksize(params) > SIZE_16M) {
		spi_flash_reset_4byte_winbond(spi, 1);
		params->four_byte = 1;
	} else
		params->four_byte = 0;
