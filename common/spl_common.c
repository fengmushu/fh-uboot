...
unsigned int *boot_params_ptr = NULL;
...
void spl_set_header_raw_uboot(void)
{
	...
	spl_image.entry_point = CONFIG_SYS_UBOOT_BASE;
	spl_image.load_addr = CONFIG_SYS_UBOOT_BASE;
...
{
	unsigned int header_size = sizeof(struct image_header);
...
static unsigned int spl_boot_list[] = {
	BOOT_DEVICE_NONE,
	BOOT_DEVICE_NONE,
	BOOT_DEVICE_NONE,
	BOOT_DEVICE_NONE,
...
__weak void board_boot_order(unsigned int *spl_boot_list)
{
...
static void announce_boot_device(unsigned int boot_device)
{
	int i;
...
#else
static inline void announce_boot_device(unsigned int boot_device) { }
#endif

static int spl_load_image(unsigned int boot_device)
{
	switch (boot_device) {
#ifdef CONFIG_SPL_RAM_DEVICE
...
void preloader_console_init(void)
{
	...
	puts("\nU-Boot SPL " U_BOOT_VERSION " (" U_BOOT_DATE " - " \
			U_BOOT_TIME ")\n");
